...
/**
 * Creates a zookeeper client and fetches metadata for brokers and then creates connections to them
 *
 * @fires Client#brokersChanged
 * @fires Client#ready
 *
 * @private
 */
Client.prototype._connect = function () {
    ...
    self.zk = new Zookeeper(self.connectionString, self.zkOptions)

    self.zk.on('brokersChanged', function (brokers) {
        self.brokerMetadata = brokers
        self._refreshBrokers()
        //TODO: Should probably wait for the cb
...
/**
 * Connects and destroys existing brokers based on changes to current broker metadata from zookeeper
 *
 * @private
 */
Client.prototype._refreshBrokers = function () {
    var self = this,
        newBrokers = {},
        brokerAddress

    //Add new broker connections
    _.values(self.brokerMetadata).forEach(function (broker) {
        brokerAddress = broker.host + ':' + broker.port

        if (self.brokers.hasOwnProperty(brokerAddress) === false) {
            self.brokers[brokerAddress] = self.createBroker(broker.host, broker.port);
        }

        newBrokers[brokerAddress] = true
    })

    //Remove any dead brokers
    Object.keys(self.brokers).forEach(function (broker) {
        if (newBrokers.hasOwnProperty(broker) === false) {
            self.brokers[broker].close()
            delete self.brokers[broker]
        }
    })
}
...
Client.prototype.createBroker = function connect(host, port, longpolling) {
    ...
    socket.on('close', function (err) {
        //TODO: keep from retrying dead brokers
        self.emit('close');
        retry(this);
    });
...
/**
 * Notifies watchers that the producer is ready
 *
 * @event Client#ready
 */

/**
 * Notifies watchers that the brokers have changed, this is not really an actionable event. Useful for logging
 *
 * @event Client#brokersChanged
 */